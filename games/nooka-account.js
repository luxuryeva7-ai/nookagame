...
(function () {
  ...
  /* Локально сервер крутится рядом на :8787. Имя хоста берём как есть:
     ...
     На тестовом стенде ветки (<ветка>.dev.nookagame.ru) сайт и сервер
     живут на одном домене — пустой адрес значит «тот же хост». Так
     cookie одной ветки не видна соседним и не нужен CORS. */
  var API = (function () {
    var h = location.hostname;
    if (h === 'localhost' || h === '127.0.0.1') return 'http://' + h + ':8787';
    if (/\.dev\.nookagame\.ru$/.test(h)) return '';
