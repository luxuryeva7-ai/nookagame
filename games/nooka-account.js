/* ============================================================
   ...
   Сервер — на том же домене, что и сайт (репозиторий nookagame-api). Сессия
...
(function () {
  ...
  /* Сервер живёт на том же домене, что и страница — и на боевом сайте,
     ...
     к соседним веткам, ни с боевого сайта на стенды, и CORS не нужен.

     Исключение одно — разработка: локально сервер крутится рядом на
     :8787. Имя хоста берём как есть, потому что localhost и 127.0.0.1
     для браузера разные сайты, и cookie с одного на другой не поедет. */
  var API = (function () {
    var h = location.hostname;
    return (h === 'localhost' || h === '127.0.0.1') ? 'http://' + h + ':8787' : '';
  })();
