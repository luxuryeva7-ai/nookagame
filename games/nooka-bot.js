...
(function () {
  ...
  function botSVG(o, opts) {
    ...
    /* xmlns обязателен: без него робот не грузится как картинка
       (data:image/svg+xml) — а именно так он попадает на карточку героя */
  ...
  function add(roots, patch, label, group, say) {
    /* label — как деталь называется на кнопке, say — как она звучит
       внутри фразы: «глаза-звёзды» на панели, «со звёздными глазами»
       в описании, которое собирается из выбранного. */
    WORDS.push({ roots: roots, patch: patch, label: label, group: group, say: say || label });
  }
  ...
  add(['обычн размер', 'средн'], { size: 'normal' }, 'обычный', 'размер');

  add(['звёздн', 'звездн', 'звёзды в глаз', 'глаза звёзд'], { eye: 'star' }, 'глаза-звёзды', 'глаза', 'со звёздными глазами');
  add(['сердечк', 'влюбл'], { eye: 'heart' }, 'глаза-сердечки', 'глаза', 'с глазами-сердечками');
  add(['полумесяц', 'месяц', 'сонн'], { eye: 'moon' }, 'глаза-полумесяцы', 'глаза', 'с глазами-полумесяцами');
  add(['хитр', 'прищур', 'подмиг'], { eye: 'wink' }, 'хитрые глаза', 'глаза', 'с хитрыми глазами');
  add(['крестик', 'зажмур'], { eye: 'cross' }, 'глаза-крестики', 'глаза', 'с зажмуренными глазами');
  add(['одноглаз', 'один глаз', 'циклоп'], { eye: 'one' }, 'один глаз', 'глаза', 'с одним глазом');
  add(['квадратн глаз', 'глаза квадрат'], { eye: 'sq' }, 'квадратные глаза', 'глаза', 'с квадратными глазами');
  add(['кольц'], { eye: 'ring' }, 'глаза-кольца', 'глаза', 'с глазами-кольцами');
  add(['точечн', 'глаза точк'], { eye: 'dot' }, 'глаза-точки', 'глаза', 'с глазами-точками');
  add(['глаза дуг', 'дуги'], { eye: 'arc' }, 'глаза-дуги', 'глаза', 'с глазами-дугами');
  add(['глаза кругл', 'кругл глаз'], { eye: 'round' }, 'круглые глаза', 'глаза', 'с круглыми глазами');
  ...
  add(['поёт', 'поет', 'пою'], { mouth: 'o' }, 'поёт', 'рот', 'который поёт');
  add(['молчал', 'без рта'], { mouth: 'none' }, 'без рта', 'рот');
  add(['улыб'], { mouth: 'smile' }, 'с улыбкой', 'рот');
  add(['рот волн', 'волнист рот'], { mouth: 'wave' }, 'рот волной', 'рот', 'с волнистым ртом');
  add(['зубаст', 'решётк', 'решетк'], { mouth: 'grid' }, 'зубастый', 'рот');
  add(['рот полоск', 'ровн рот'], { mouth: 'line' }, 'рот полоской', 'рот', 'с ровным ртом');
  ...
  add(['лета', 'парит', 'парящ', 'левитир'], { base: 'float' }, 'летает', 'низ', 'который летает');
  ...
  add(['без шляп', 'без шапк'], { hat: 'none' }, 'без шляпы', 'шляпа');
  add(['без узор', 'гладк'], { skin: 'none' }, 'гладкий', 'узор');
  add(['без рук', 'безрук'], { arm: 'none' }, 'без рук', 'руки');
  add(['ничего не нос', 'без вещ'], { gear: 'none' }, 'без вещей', 'вещи');
  ...
  /* ── все варианты по группам: для видимой панели ──────────
     Панель показывает то же, что понимает разбор фразы, — значит
     собирается из одного словаря, а не переписывается руками. */
  function variants() {
    var out = {}, seen = {};
    WORDS.forEach(function (w) {
      var key = w.group + '|' + JSON.stringify(w.patch);
      if (seen[key]) return;
      seen[key] = 1;
      if (!out[w.group]) out[w.group] = [];
      out[w.group].push({ label: w.label, patch: w.patch, say: w.say });
    });
    return out;
  }

  /* ── фраза по выбранным деталям ──────────────────────────
     Ребёнок нажимает на детали, а в строке появляется то, как об этом
     просят словами: панель и промпт — одно и то же, сказанное дважды. */
  var SAY_ORDER = ['размер', 'цвет', 'форма', 'настроение', 'глаза', 'рот', 'уши',
    'антенна', 'шляпа', 'узор', 'вещи', 'руки', 'низ', 'свечение'];

  function describe(spec) {
    var vs = variants(), said = [];
    SAY_ORDER.forEach(function (g) {
      (vs[g] || []).forEach(function (v) {
        if (said.length && said[said.length - 1].group === g) return;   // одно на группу
        var fits = Object.keys(v.patch).every(function (k) { return spec[k] === v.patch[k]; });
        if (!fits) return;
        /* про обычное не просят: «без шляпы», «гладкий», «на колёсах» — это
           и так то, что получится само. В заказ идёт лишь то, что отличает
           робота от простого. Цвет называем всегда: его замечают первым. */
        var plain = g !== 'цвет' && Object.keys(v.patch).every(function (k) {
          return DEF[k] === v.patch[k];
        });
        if (plain) return;
        said.push({ group: g, label: v.say || v.label });
      });
    });
    var head = [], tail = [];
    said.forEach(function (s) {
      /* «синий», «круглый» — перед словом «робот»; «в короне» — после */
      if (['размер', 'цвет', 'форма', 'настроение'].indexOf(s.group) > -1) head.push(s.label);
      else tail.push(s.label);
    });
    return (head.join(' ') + ' робот ' + tail.join(' ')).replace(/\s+/g, ' ').trim();
  }
